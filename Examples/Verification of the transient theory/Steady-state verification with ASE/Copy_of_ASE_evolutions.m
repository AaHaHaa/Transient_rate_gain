...
gain_rate_eqn.woPulse = struct('lambda',[604.8341032377740,3475.425895855164],'Nt',2^11);
...
h = plot(t1/1e3,[ASE_forward,avg_ASE_forward,ASE.forward(end)*ones(gain_rate_eqn.woPulse.Nt,1)],'linewidth',2);
...
h = plot(t1/1e3,[ASE_backward,avg_ASE_backward,ASE.backward(1)*ones(gain_rate_eqn.woPulse.Nt,1)],'linewidth',2);
set(h(1),'Color','b'); set(h(2),'Color','r'); set(h(3),'Color','k');
set(gca,'fontsize',20);
xlabel('Time (\mus)'); ylabel('Power (mW)');
legend('|A(t)|^2','smoothed');
...
%% Save results
save('transient_ASE_evolutions2.mat','f','t','prop_output','sim','fiber','gain_rate_eqn');