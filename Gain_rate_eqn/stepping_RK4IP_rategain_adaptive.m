function [A1t,a5,...
          Power_pump_forward,...
          opt_dz,success,...
          N,dN] = stepping_RK4IP_rategain_adaptive(A0t_forward,Power_pump_forward,N,...
                                                   Nt,dt,num_windows,...
                                                   sim,gain_rate_eqn,...
...
%
% Input:
%    A0t_forward - initial forward-propagating field in the time domain (Nt,1); sqrt(W)
%    Power_pump_forward - (Nt,1); the power of the co-propagating pump
%    N - (Nt,num_levels-1); populations
%
%    Nt - the number of sampling points; a (1,2) array with Nt of time windows, with or without the pulse
%    dt - time grid point spacing; ps;  a (1,2) array with dt of time windows, with or without the pulse
%
%    sim
%    gain_rate_eqn
%
...
%    D_op - dispersion term D (Nt,1)
%
%    a5_1 - the RK4 term that is sent from the previous step; (Nt,1)
%    dummy_var - unused variable
%
% Output:
%    A1t - the field (in the time domain) after one step size; (Nt,1)
%    a5 - the RK4 term that can be reused in the next step; (Nt,1)
%    Power_pump_forward - a (1,2) cell array of the power of the co-propagating pump; (Nt,1)
%    opt_dz - recommended step size
%    success - whether the current step size is sufficiently small for the required tolerance
%    N - (Nt,num_levels-1); the ion density/population of each energy level
...
A1t = initialize_zeros(Nt,num_windows);
for window_i = 2:2:num_windows
    ...
    A1t{window_i} = fft(A1,[],1);
    
    % Downsample it back to the original time window
    A1t{window_i} = A1t{window_i}(1:Nt);
end
