...
gain_rate_eqn.copump_power = 7.7; % W
...
grating_spacings = linspace(0.8,1.4,201)*1e-3;%grating_spacing*linspace(0.9,1.1,101);
...
% Note below that we ignore temporal interference in spectrogram for
% visualization of how spectra vary in different pulses.
% In principle, we shouldn't ignore interference.
ignore_temporal_interference = true;
calc_spectrogram(t,f,prop_output(2).fields.forward(:,:,end),ignore_temporal_interference,[-380,380],[990,1160],200);
print(gcf,'burst_mode_GMNA_spectrogram.jpg','-djpeg');
