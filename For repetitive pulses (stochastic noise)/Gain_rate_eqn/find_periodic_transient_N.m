...
%   Nt: a scalar; the number of points in each window
%   dt: a (1,num_windows) array; the temporal sampling period (ps)
...
%   At_backward: a (1,1,1,num_windows) cell array, each containing the backward field at its time window
%   Power_pump_forward: a (1,1,1,num_windows) cell array, each containing the forward pump power at its time window
%   Power_pump_backward: a (1,1,1,num_windows) cell array, each containing the backward pump power at its time window
%
% Output:
...
for window_i = 1:num_windows
    ...
    % Compute F^(-1)[A(Omega) * sqrt(sigma/hbar/omega)]
    At_backward_i = fft(extended_Aw_backward{window_i}.*sqrt(gain_rate_eqn.extended_cross_sections{mod(window_i+1,2)+1}./gain_rate_eqn.extended_E_photon{mod(window_i+1,2)+1}),[],1); % sqrt(m^2/s); size: (Nt,num_cross_sections)
    
    A2_i = abs(At_forward_i).^2 + abs(At_backward_i).^2; % A2(t); the number of scattered photons

    % Rate/photon_energy:
    % size: (Nt,num_cross_sections), unit: W
    num_cross_sections = length(gain_rate_eqn.cross_sections_pump);
    if sim.gpu_yes
        R_forward  = zeros([gain_rate_eqn.acyclic_conv_stretch(Nt),num_cross_sections],'gpuArray');
        R_backward = zeros([gain_rate_eqn.acyclic_conv_stretch(Nt),num_cross_sections],'gpuArray');
    else
        R_forward  = zeros([gain_rate_eqn.acyclic_conv_stretch(Nt),num_cross_sections]);
        R_backward = zeros([gain_rate_eqn.acyclic_conv_stretch(Nt),num_cross_sections]);
    end
    if ismember(gain_rate_eqn.pump_direction,{'co','bi'})
        R_forward  = gain_rate_eqn.overlap_factor.pump.*gain_rate_eqn.cross_sections_pump.*extended_Power_pump_forward{window_i} /h*(gain_rate_eqn.pump_wavelength*1e-9)/c; % 1/s
    end
    if ismember(gain_rate_eqn.pump_direction,{'counter','bi'})
        R_backward = gain_rate_eqn.overlap_factor.pump.*gain_rate_eqn.cross_sections_pump.*extended_Power_pump_backward{window_i}/h*(gain_rate_eqn.pump_wavelength*1e-9)/c; % 1/s
    end
    R_pump = R_forward + R_backward;
...
if length(gain_rate_eqn.energy_levels) > 2 % don't run it for a two-level system; just run fine optimization
    ...
    % Rough optimization has been done, so reduce the variation step to 0.5% for a latter finer optimization
    usual_delta = 0.005;
...
% and outputs the error between N(t=0 in window 1) and N(t=t_end in the last window),
% as well as the computed populations.
%
