...
% initial_condition takes only the last field
for window_i = 1:num_windows
    initial_condition(window_i).fields.forward = initial_condition(window_i).fields.forward(:,:,end);
end
...
for window_i = 1:num_windows
    if mod(window_i,2) == 1
        if gain_rate_eqn.ignore_ASE
            At_noise{1,window_i} = zeros(gain_rate_eqn.acyclic_conv_stretch(Nt),1);
        else
            At_noise{1,window_i} = shot_noise(gain_rate_eqn.acyclic_conv_stretch(Nt),dt(window_i),sim.incoherent.f0,cs(window_i));
        end
    else
        At_noise{1,window_i} = shot_noise(gain_rate_eqn.acyclic_conv_stretch(Nt),dt(window_i),sim.f0,cs(window_i));
    end

    if sim.gpu_yes
        At_noise{1,window_i} = gpuArray(At_noise{1,window_i});
    end
end
...
foutput = struct('z', sim.dz*(0:num_zSteps)',...
                 'fields', A_out,...
                 'dt', num2cell(dt),...
                 'betas', sim.betas,...
                 'seconds', fulltime,...
