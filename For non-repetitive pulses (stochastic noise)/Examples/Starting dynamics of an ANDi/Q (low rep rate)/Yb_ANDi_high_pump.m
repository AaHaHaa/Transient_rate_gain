...
%%
load('../steady state/ss_for_initial_N_1MHz_high_pump.mat');
...
%%
gain_rate_eqn.woPulse = struct('lambda',[1010,1050],'Nt',2^10);
...
%% cavity parameters
max_rt = 5e3; % maximum roundtrips (in case it doesn't converge)
each_max_rt = 1000;
...
save('oscillator_general_info_high_pump.mat');
...
while rt_num <= max_rt
    ...
    while rt_num_thousand ~= 0
        ...
        prop_output(1).fields.forward = last_fields1 + initial_condition(1).fields.forward*sqrt(1e-3/sum(abs(initial_condition(1).fields.forward).^2)/initial_condition(1).dt).*exp(1i*2*pi*rand(woPulse.Nt,1));
        prop_output(2).fields.forward = last_fields2 + initial_condition(2).fields.forward*sqrt(1e-3*(dt/dt_woPulse)/sum(abs(initial_condition(2).fields.forward).^2)/initial_condition(2).dt).*exp(1i*2*pi*rand(Nt,1));
        ...
        tmp2 = struct('dt',prop_output(2).dt,'fields',prop_output(2).fields.forward);
        if rt_num < 30000
            saturation_power_Q = min(0.01,max(abs(tmp2.fields).^2));
        else
            saturation_power_Q = min(1,max(abs(tmp2.fields).^2));
        end
        tmp1 = saturable_absorber_action_simple(tmp1, saturation_power_Q, moddepth);
        saturation_power = 2500;%min(2500,max(abs(tmp2.fields).^2));
        tmp2 = saturable_absorber_action_simple(tmp2, saturation_power, moddepth);
    ...
    close all;
    save(sprintf('oscillator_high_pump_%u.mat',rt_num-1),'output_field','output_energy','population1','population2','population3','population4','pump1','pump2','pump3','pump4');
end