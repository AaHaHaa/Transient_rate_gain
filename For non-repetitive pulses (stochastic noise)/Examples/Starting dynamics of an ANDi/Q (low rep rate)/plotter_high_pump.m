...
for i = 1:length(name_idx)
    load(sprintf('oscillator_high_pump_%u.mat',name_idx(i)));
...
plot(ax1,rt/1e3,energy_all/1e3,'Color','b','linewidth',4); ylabel(ax1,'Energy (\muJ)'); xlim(ax1,[0,0.6]); ylim(ax1,[0,8]);
% Populations
h = plot(ax2,rt/1e3,[population1_all,population2_all,population3_all,population4_all]*100,'linewidth',4); ylabel(ax2,'N_1 (%)'); xlim(ax2,[0,0.6]); ylim(ax2,[0,55]); set(ax2, 'YAxisLocation', 'right'); % set y-axes to right side
set(h(1),'Color',0*ones(1,3)); set(h(2),'Color',0.4*ones(1,3)); set(h(3),'Color',0.6*ones(1,3)); set(h(4),'Color',0.7*ones(1,3));
xlabel(ax1,'Roundtrip (x10^3)'); xlabel(ax2,'');
set([ax1,ax2],'Color', 'None','fontsize',20);
set(ax1,'YColor','b'); set(ax2,'YColor','k');
pos = get(gcf,'Position'); set(gcf,'Position',[pos(1:2),pos(3),pos(4)]); % make the figure "wider"
pos = ax1.Position; set([ax1,ax2], 'Position', [0.1,pos(2),0.77,pos(4)]); % make the axes 10% more narrow
% Extend the ax1 y-axes ticklabels rightward a bit by adding space
exportgraphics(gcf, 'evolution_high_pump.pdf', 'ContentType', 'vector'); % print command doesn't work for figures with a far-away ylabel