...
for i = 1:length(name_idx)
    load(sprintf('oscillator_low_pump_%u.mat',name_idx(i)));
...
plot(ax1,rt/1e3,energy_all/1e3,'Color','b','linewidth',4); ylabel(ax1,'Energy (\muJ)'); xlim(ax1,[0,2]); ylim(ax1,[0,3]);
% Populations
h = plot(ax2,rt/1e3,[population1_all,population2_all,population3_all,population4_all]*100,'linewidth',4); ylabel(ax2,'N_1 (%)'); xlim(ax2,[0,2]); ylim(ax2,[0,55]); set(ax2, 'YAxisLocation', 'right'); % set y-axes to right side
...
exportgraphics(gcf, 'evolution_low_pump.pdf', 'ContentType', 'vector'); % print command doesn't work for figures with a far-away ylabel