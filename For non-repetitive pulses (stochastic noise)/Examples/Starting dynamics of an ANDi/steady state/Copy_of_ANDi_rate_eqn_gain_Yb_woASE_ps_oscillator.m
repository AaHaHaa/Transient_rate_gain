...
rmpath('../../../GMMNLSE algorithm/','../../../user_helpers/')
...
gain_rate_eqn.copump_power = 0.2; % W
...
tfwhm = 1.5; % ps
total_energy = 0.00000000000000000003; % nJ
...
L_air = 29.8329; % 1 is the free-space length
c = 299792458; % m/s
...
while rt_num < max_rt
    ...
    func2 = calc_chirp;
    [chirp,chirped_pulse] = func2.Gaussian( 2,ifftshift(2*pi*f),ifft(prop_output.fields(:,:,end)),1);
    prop_output.fields = fft(ifft(prop_output.fields(:,:,end)).*exp(1i*0.2/2*2*pi*ifftshift(f-sim.f0).^2));
    ...
    % -----------------------------------------------------------------
    % Saturable absorber
    saturation_power = 2500;%min(2500,max(abs(prop_output.fields(:,:,end)).^2));
...
save('ss_for_initial_N_ps_oscillator.mat');