...
plot(ax1,rt/1e3,energy_all/1e3,'Color','b','linewidth',4); ylabel(ax1,'Energy (\muJ)'); xlim(ax1,[0,5]); ylim(ax1,[0,3]);
...
linkaxes([ax1,ax2,ax3],'x'); % link the x axes
% Energy and peak power
plot(ax1,rt/1e3,energy_all/1e3,'Color','b','linewidth',9); xlim(ax1,[0,0.02]); ylim(ax1,[0,3]); set(ax1,'YTick',[]);
plot(ax2,rt/1e3,peak_power_all/1e3,'Color','r','linewidth',9); xlim(ax2,[0,0.02]); ylim(ax2,[0,90]); set(ax2,'YTick',[]);
% Populations
h = plot(ax3,rt/1e3,[population1_all,population2_all,population3_all,population4_all]*100,'linewidth',10); xlim(ax3,[0,0.02]); ylim(ax3,[0,55]); set(ax3,'YTick',[]); set(ax3, 'YAxisLocation', 'right'); % set y-axes to right side
set(h(1),'Color',0*ones(1,3)); set(h(2),'Color',0.4*ones(1,3)); set(h(3),'Color',0.6*ones(1,3)); set(h(4),'Color',0.7*ones(1,3));
set([ax1,ax2,ax3],'Color', 'None','fontsize',60);
set(ax1,'YColor','b'); set(ax2,'YColor','r'); set(ax3,'YColor','k');
pos = get(gcf,'Position'); set(gcf,'Position',[pos(1:2),pos(3),pos(4)]); % make the figure "wider"
pos = ax1.Position; set([ax1,ax2,ax3], 'Position', [0.143,pos(2),0.77,pos(4)]); % make the axes 10% more narrow
% Extend the ax1 y-axes ticklabels rightward a bit by adding space
ax1.YTickLabel = strcat(ax1.YTickLabel,{'            '});
exportgraphics(gcf, 'evolution (closeview).pdf', 'ContentType', 'vector'); % print command doesn't work for figures with a far-away ylabel
%}