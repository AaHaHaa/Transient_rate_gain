...
%% cavity parameters
max_rt = 10000; % maximum roundtrips (in case it doesn't converge)
each_max_rt = 1000;
...
while rt_num <= max_rt
    ...
    while rt_num_thousand ~= 0
        ...
        saturation_power_Q = min(2500,max([max(abs(tmp1.fields).^2,max(abs(tmp2.fields).^2))]));
        tmp1 = saturable_absorber_action_simple(tmp1, saturation_power_Q, moddepth);
        saturation_power = min(2500,max([max(abs(tmp1.fields).^2,max(abs(tmp2.fields).^2))]));
        tmp2 = saturable_absorber_action_simple(tmp2, saturation_power, moddepth);
    ...
    close all;
    save(sprintf('oscillator2_%u.mat',rt_num-1),'output_field','output_energy','population1','population2','population3','population4','pump1','pump2','pump3','pump4');
end