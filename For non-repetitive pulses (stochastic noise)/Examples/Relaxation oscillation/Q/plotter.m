...
linkaxes([ax1,ax2,ax2],'x'); % link the x axes
% Energy and peak power
plot(ax1,rt/1e3,energy_all/1e3,'Color','b','linewidth',4); ylabel(ax1,'Energy (\muJ)'); xlim(ax1,[0,total_rt/1e3]); ylim(ax1,[0,0.5]);
% Populations
h = plot(ax2,rt/1e3,[population1_all,population2_all,population3_all,population4_all]*100,'linewidth',4); ylabel(ax2,'N_1 (%)'); xlim(ax2,[0,total_rt/1e3]); ylim(ax2,[0,15]); set(ax2, 'YAxisLocation', 'right'); % set y-axes to right side
...
pos = ax1.Position; set([ax1,ax2], 'Position', pos);
