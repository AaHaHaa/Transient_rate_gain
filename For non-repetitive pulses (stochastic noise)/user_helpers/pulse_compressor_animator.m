...
%       Ilim - (1,2) matrix; the range of the power to plot (nm) (default: [])
%       filename
...
[tlim,Ilim,filename,m] = optargs{:};

switch compressor_type
    case {'Treacy-beta2','Treacy-r','Treacy-t'}
        Frame = Treacy(compressor_type,separation,theta_in,wavelength0,time,field,grating_spacing,                     m,tlim,Ilim);
    case 'prism'
        Frame = prism(                 separation,         wavelength0,time,field,               alpha,prism_material,  tlim,Ilim);
    case {'grism1','grism2'}
        Frame = grism(compressor_type,separation,theta_in,wavelength0,time,field,grating_spacing,alpha,prism_material,m,tlim,Ilim);
    case 'Offner1'
        Frame = Offner(compressor_type,separation,theta_in,wavelength0,time,field,grating_spacing,R,0,                 m,tlim,Ilim);
    case 'Offner2'
        Frame = Offner(compressor_type,separation,theta_in,wavelength0,time,field,grating_spacing,R,offcenter,         m,tlim,Ilim);
    case 'Offner3'
        Frame = Offner(compressor_type,separation,theta_in,wavelength0,time,field,grating_spacing,R,0,                 m,tlim,Ilim);
    case 'Martinez'
        Frame = Martinez(              separation,theta_in,wavelength0,time,field,grating_spacing,focal_length,          tlim,Ilim);
    otherwise
        error('The value of compressor_type is wrong.');
end
...
close(fig);
set(fig_movie.Parent,'position',[fp(1) screen_size(4)-original_top-fp(4)*5.5/4 fp(3)*5.5/4 fp(4)*5.5/4]); % enlarge the figure size to fit in so many subplots
...
%%
function Frame = Treacy(compressor_type,separation,theta_in,wavelength0,time,field,grating_spacing,m,tlim,Ilim)
%GRATING_PAIR
...
for si = 1:save_point
    ...
    fig = figure;
    plot(time,abs(dechirped_field).^2/1e3,'linewidth',2,'Color','b');
